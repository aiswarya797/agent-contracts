...
def command_for_condition(
    *,
    ...
    if args.evaluate:
        common_args.extend(["--evaluate", "--eval-bench", eval_bench.as_posix()])
    if condition["agent"]:
        common_args.extend(
            [
                "--codex-command",
...
def run(args: argparse.Namespace) -> int:
    ...
    summary = {
        ...
        "condition_timeout": args.condition_timeout,
        "conditions": [
    ...
    for item in commands:
        ...
        try:
            ...
            print(
                f"Condition {item['condition']} timed out after {args.condition_timeout}s",
                file=sys.stderr,
            )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--condition-timeout",
        type=int,
        help="Maximum seconds to allow each condition subprocess to run. Defaults to max(--codex-timeout + 60, 300).",
    )
    parser.add_argument(
        "--spark-codex-command",
    ...
    if args.condition_timeout is None:
        args.condition_timeout = max(args.codex_timeout + 60, 300)
    if args.condition_timeout < 1:
        parser.error("--condition-timeout must be >= 1")
    return args
